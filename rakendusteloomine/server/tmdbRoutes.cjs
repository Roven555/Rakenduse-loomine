...
// Excluded categories
const EXCLUDED_CATEGORIES = ['Dokumentaal', 'Documentary'];
...
// Function to check if a movie has excluded categories
function hasExcludedCategory(movie) {
  // Check genre_ids (numbers)
  if (movie.genre_ids && movie.genre_ids.includes(99)) return true;
  
  // Check genres (objects)
  if (Array.isArray(movie.genres) && movie.genres.some(g => g.name === 'Documentary')) return true;
...
router.get('/movies/popular', async (req, res) => {
  try {
    ...
    const movies = (data.results || [])
      .filter((m) => m.poster_path && m.backdrop_path && !isMovieExcluded(m) && !hasExcludedCategory(m))
...
router.get('/actors/:id/movies', async (req, res) => {
  try {
    ...
    const movies = (data.cast || [])
      .filter((m) => m.poster_path && !isMovieExcluded(m) && !hasExcludedCategory(m))
